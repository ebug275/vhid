#include "driver.h"
#include "gamepadpdo.tmh"

#ifdef ALLOC_PRAGMA
#pragma alloc_text(PAGE, GamepadCreatePdo)
#endif

UCHAR device_descriptor_gamepad[] = {
    // 18 bytes
    0x12,        // bLength
    ...
    LOBYTE(GAMEPAD_PRODUCT_ID),
    HIBYTE(GAMEPAD_PRODUCT_ID),  // idProduct 0x0820
...
UCHAR configuration_descriptor_gamepad[] = {
    // 41 bytes
    0x09,        // bLength
...
UCHAR report_desciptor_gamepad[] = {
    // 44 bytes
...
UCHAR gamepad_strings[][64] = {
    { 0x04, 0x03, 0x09, 0x00 },
    { 0x16, 0x03, 0x4D, 0x00, 0x46, 0x00, 0x52, 0x00, 0x5F, 0x00, 0x53, 0x00, 0x54, 0x00, 0x52, 0x00,
      0x49, 0x00, 0x4E, 0x00, 0x47, 0x00 }, // MFR_STRING
    { 0x1A, 0x03, 0x56, 0x00, 0x48, 0x00, 0x49, 0x00, 0x44, 0x00, 0x5F, 0x00, 0x47, 0x00, 0x17, 0x00,
      0x4D, 0x00, 0x45, 0x00, 0x50, 0x00, 0x41, 0x00, 0x44, 0x00 }, // MOCON_GAMEPAD
...
)
{
    ...
    DECLARE_CONST_UNICODE_STRING(hardwareId, GAMEPAD_HARDWARE_ID);
    DECLARE_CONST_UNICODE_STRING(compatId, GAMEPAD_COMPATIBLE_ID);
    DECLARE_CONST_UNICODE_STRING(deviceLocation, L"vhiddev 0");
    ...
    // Instance ID 지정
    status = RtlUnicodeStringPrintf(&buffer, L"%02d", HARDWARE_TYPE_GAMEPAD);
    if (!NT_SUCCESS(status)) {
    ...
    status = RtlUnicodeStringPrintf(&buffer, L"VHIDDEV_Device_%02d", HARDWARE_TYPE_GAMEPAD);
    if (!NT_SUCCESS(status)) {
    ...
    pdoData->HardwareType = HARDWARE_TYPE_GAMEPAD;
    pdoData->FdoDevice = Device;
    pdoData->DeviceDescriptor = device_descriptor_gamepad;
    pdoData->ConfigurationDescriptor = configuration_descriptor_gamepad;
    pdoData->ReportDescriptor = report_desciptor_gamepad;
    ...
    pnpCaps.Address = HARDWARE_TYPE_GAMEPAD;
    pnpCaps.UINumber = HARDWARE_TYPE_GAMEPAD;
    ...
    ChildInterface.GetCrispinessLevel = GamepadGetCrispinessLevel;
    ChildInterface.SetCrispinessLevel = GamepadSetCrispinessLevel;
    ChildInterface.IsSafetyLockEnabled = GamepadIsSafetyLockEnabled;

    WDF_QUERY_INTERFACE_CONFIG_INIT(&qiConfig, (PINTERFACE)&ChildInterface, &GUID_VHIDDEV_INTERFACE_GAMEPAD, NULL);
    ...
    status = GamepadQueueInitialize(hChild);
    if (!NT_SUCCESS(status)) {
        TraceEvents(TRACE_LEVEL_ERROR, TRACE_PDO, "%!FUNC! GamepadQueueInitialize failed %!STATUS!", status);
        goto Cleanup;
    }
    ...
    fdoData->PdoChilds[HARDWARE_TYPE_GAMEPAD] = hChild;
...
)
{
    ...
    queueConfig.EvtIoInternalDeviceControl = GamepadEvtIoInternalDeviceControl;
    queueConfig.EvtIoStop = GamepadEvtIoStop;
...
)
{
    ...
    WDFREQUEST request;
    PPDO_DEVICE_DATA pdoData = NULL;
    PGAMEPAD_REPORT gamepad_report = NULL;
    PVOID buffer = NULL;
    size_t bufSize = 0;
    ...
    status = WdfRequestRetrieveInputBuffer(Request, sizeof(GAMEPAD_REPORT), &buffer, &bufSize);
    if (!NT_SUCCESS(status) || bufSize < sizeof(GAMEPAD_REPORT)) {
        TraceEvents(TRACE_LEVEL_ERROR, TRACE_PDO, "%!FUNC! not enough input request buffer failed");
        return status;
    }

    gamepad_report = (PGAMEPAD_REPORT)buffer;

    // todo: gamepad report data update
    gamepad_report->report_id = 1;  // always 1
    ...
    if (NT_SUCCESS(status)) {
        ...
        if (urb->UrbHeader.Function == URB_FUNCTION_BULK_OR_INTERRUPT_TRANSFER) {
            if (urb->UrbBulkOrInterruptTransfer.TransferBufferLength >= sizeof(GAMEPAD_REPORT)) {
                RtlCopyMemory(urb->UrbBulkOrInterruptTransfer.TransferBuffer, gamepad_report, sizeof(GAMEPAD_REPORT));
                urb->UrbBulkOrInterruptTransfer.TransferBufferLength = sizeof(GAMEPAD_REPORT);
...
VOID
GamepadEvtIoInternalDeviceControl(
    _In_ WDFQUEUE    Queue,
    _In_ WDFREQUEST  Request,
...
)
{
    ...
    case IOCTL_INTERNAL_USB_SUBMIT_URB:
        urb = params.Parameters.Others.Arg1;
        status = GamepadUrbProcess(pdoData, Request, urb);
        break;
    }
...
)
{
    ...
    case URB_FUNCTION_SELECT_CONFIGURATION:
        ...
        status = GamepadSelectConfiguration(PdoData, Request, Urb);
        break;

    case URB_FUNCTION_GET_DESCRIPTOR_FROM_DEVICE:
        TraceEvents(TRACE_LEVEL_INFORMATION, TRACE_PDO, "URB_FUNCTION_GET_DESCRIPTOR_FROM_DEVICE");
        status = GamepadGetUsbDescriptor(PdoData, Request, Urb);
        break;
    ...
    case URB_FUNCTION_CLASS_INTERFACE:
        ...
        status = GamepadProcessClassInterface(PdoData, Request, Urb);
        break;

    case URB_FUNCTION_CONTROL_TRANSFER_EX:
        TraceEvents(TRACE_LEVEL_INFORMATION, TRACE_PDO, "URB_FUNCTION_CONTROL_TRANSFER_EX");
        status = GamepadProcessControlTransfer(PdoData, Request, Urb);
        break;
...
)
{
    ...
    if (Urb->UrbSelectConfiguration.ConfigurationDescriptor != NULL) {
        ...
        Urb->UrbSelectConfiguration.ConfigurationHandle = &configuration_descriptor_gamepad[0];

        USB_INTERFACE_DESCRIPTOR* intf = (USB_INTERFACE_DESCRIPTOR*)&configuration_descriptor_gamepad[9];
        ...
        USB_ENDPOINT_DESCRIPTOR* endp = (USB_ENDPOINT_DESCRIPTOR*)&configuration_descriptor_gamepad[27];
        ...
        USB_ENDPOINT_DESCRIPTOR* endp2 = (USB_ENDPOINT_DESCRIPTOR*)&configuration_descriptor_gamepad[34];
        intf_info->Pipes[1].MaximumPacketSize = endp2->wMaxPacketSize;
        intf_info->Pipes[1].EndpointAddress = endp2->bEndpointAddress;
        intf_info->Pipes[1].Interval = endp2->bInterval;
        intf_info->Pipes[1].PipeType = UsbdPipeTypeInterrupt;
        intf_info->Pipes[1].PipeHandle = endp2;
    }
...
)
{
    ...
    case USB_DEVICE_DESCRIPTOR_TYPE:
        ...
        length = sizeof(device_descriptor_gamepad);
        ...
        DumpMemory((PUCHAR)&device_descriptor_gamepad, length);
        RtlCopyMemory(ioBuffer, &device_descriptor_gamepad, length);
    ...
    case USB_CONFIGURATION_DESCRIPTOR_TYPE:
        ...
        length = sizeof(configuration_descriptor_gamepad);

        ioBuffer = Urb->UrbControlDescriptorRequest.TransferBuffer;
        if (length > Urb->UrbControlDescriptorRequest.TransferBufferLength) length = Urb->UrbControlDescriptorRequest.TransferBufferLength;

        DumpMemory((PUCHAR)&configuration_descriptor_gamepad, length);
        RtlCopyMemory(ioBuffer, &configuration_descriptor_gamepad, length);
    ...
    case USB_STRING_DESCRIPTOR_TYPE:
        ...
        length = (ULONG)gamepad_strings[Urb->UrbControlDescriptorRequest.Index][0];

        ioBuffer = Urb->UrbControlDescriptorRequest.TransferBuffer;
        if (length > Urb->UrbControlDescriptorRequest.TransferBufferLength) length = Urb->UrbControlDescriptorRequest.TransferBufferLength;

        DumpMemory(gamepad_strings[Urb->UrbControlDescriptorRequest.Index], length);
        RtlCopyMemory(ioBuffer, gamepad_strings[Urb->UrbControlDescriptorRequest.Index], length);
    ...
    case HID_REPORT_DESCRIPTOR_TYPE:
        ...
        length = sizeof(report_desciptor_gamepad);

        ioBuffer = Urb->UrbControlDescriptorRequest.TransferBuffer;
        if (length > Urb->UrbControlDescriptorRequest.TransferBufferLength) length = Urb->UrbControlDescriptorRequest.TransferBufferLength;

        DumpMemory(report_desciptor_gamepad, length);
        RtlCopyMemory(ioBuffer, report_desciptor_gamepad, length);
